...
def run_publish_message(data, context= None):
    ...
    auth_user = os.environ.get("AUTHUSER")
    ...
    # Get transcript
    title, message, transcript = load_file_from_gcs(storage_client,SRC_BUCKET_NAME,FILE)
    meeting_dict = {
        'name' : FILE[10:].replace('.json','').replace('.txt',''),
        'full_summary_url' : f"https://storage.cloud.google.com/{transcript_bucket}/{transcript}?{auth_user}",
        'message_title' : title,
...
def main():
    ...
    os.environ["AUTHUSER"] = "authuser=2"
